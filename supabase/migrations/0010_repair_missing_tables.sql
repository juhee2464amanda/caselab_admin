...
-- ============================================================
-- 0010_repair_missing_tables.sql
-- 작성일: 2026-06-08
-- 목적: Caselab-prod 스키마 드리프트 복구.
--   검증 결과 topic_suggestions / featured_contents / tags (+content_tags/tool_tags)
--   3(+2)개 테이블 누락 확인. 0003이 featured_contents·topic_suggestions를
--   ALTER/트리거 부착하므로, 두 테이블을 먼저 생성한 뒤 0003 전체를 재실행한다.
-- 멱등: create table if not exists / drop policy if exists / create or replace
--   → 기존 객체·데이터에 영향 없음 (재실행 안전).
-- 실행: Supabase Dashboard(Caselab-prod) → SQL Editor → 본 파일 전체 Run.
-- ============================================================

-- ===== PART A. topic_suggestions (원천: 0001 §13) =====
...
drop policy if exists "Anyone view suggestions" on public.topic_suggestions;
create policy "Anyone view suggestions"
  on public.topic_suggestions for select using (true);

drop policy if exists "Auth users insert suggestions" on public.topic_suggestions;
...
-- ===== PART B. featured_contents 베이스 (원천: 0002 §3) =====
...
drop policy if exists "Public read active featured" on public.featured_contents;
create policy "Public read active featured"
  on public.featured_contents for select using (active = true or public.is_admin());

drop policy if exists "Admins manage featured" on public.featured_contents;
...
-- ============================================================
-- ===== PART C. 0003 전체 재실행 (tags/content_tags/tool_tags 생성 + 나머지 멱등) =====
-- ============================================================
