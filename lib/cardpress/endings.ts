...
export function endingFor(
  ...
  if (ctaType === 'comment_dm') {
    ...
    return {
      ...
      template: 'P6',
      props: {
        // 한 문장으로 읽히게 조판한다 — 앞머리("댓글에") / 키워드 / 뒷말이 이어진 문장이다.
        // 전에는 키워드만 덩그러니 크고 아래 안내문이 따로 놀아서 두 덩어리로 읽혔다(운영자 2026-08-30).
        leadIn: '댓글에',
        // 따옴표는 "이 단어를 그대로 적으라"는 지시 — 없으면 키워드가 문장에 섞여 안 읽힌다
        big: `'${keyword}'`,
        // 230px 풀사이즈는 키워드에 너무 컸다 — 한 방은 유지하되 카드가 글자에 먹히지 않게
        bigMax: 150,
        // 안내문 오버라이드 — 빈 문자열은 "줄 삭제"인데 P6 스키마가 min(1)이라 공백 하나로 눕힌다
        resolve: (ov.resolve ?? '을 남기면 상세내용을 보내드려요') || ' ',
        accentColor,
        ...(image
          ? {
              image,
