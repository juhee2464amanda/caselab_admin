...
function captionBlockGuide(src: CardSource): string {
  ...
  const GUIDES: Record<string, string> = {
    case: `블록1 — 어떤 문제를 푸나: 이 케이스가 해결하는 문제 하나. 증상 나열이 아니라 "무엇이 안 되고 있었는지".
블록2 — 어떻게 적용했나: stepCards에서 실제로 한 일. 사람이 한 일과 AI가 한 일을 갈라서 구체적으로.
블록3 — 무엇이 좋아졌나: 적용 후 달라진 것. 숫자·시간·결과물로 말할 것.
⚠️ pros/cons("잘된 점·아쉬웠던 점")를 캡션에 옮기지 말 것 — 그건 카드(B5)의 몫이다.
   캡션이 말할 것은 **적용과 효과**다. 한계는 효과를 정직하게 만드는 선에서 한 구절만.`,
    trend: `블록1 — 무엇이 어떻게 달라졌나: 변화의 내용을 **구체적으로**. 수치·이름·전후 비교를 넣고 뭉뚱그리지 말 것.
블록2 — 그래서 뭐가 가능해졌나: 전에 못 하던 것 중 이제 되는 것. 기능 나열이 아니라 "할 수 있게 된 일"로.
블록3 — 나에게 시사하는 바: 독자가 지금 판단을 바꿔야 하는 지점. **직무 하나를 특정할 것**.
   일반론("AI 시대엔 학습이 중요합니다")이면 이 블록은 죽는다.`,
    tool: `블록1 — 왜 이걸 추천하나: 비슷한 도구가 많은데 굳이 이걸 고른 이유. 이 도구에만 있는 지점 하나.
블록2 — 누구를 위한 도구인가: 직무·상황을 구체적으로. 독자가 자기 얘기인지 판단하는 자리다.
블록3 — 어떤 일에 쓰나: 꺼내 드는 순간을 장면으로 그릴 것.
(가격·한계는 카드가 이미 한 장으로 다룬다. 무료라는 사실이 추천 이유의 핵심이면 블록1에 한 구절로만 붙인다)`,
    prompt: `블록1 — 어디서 온 프롬프트인가: 출처(공식 문서·실무자·실측 결과). **신뢰가 이 소재의 엣지다** — 출처가 있으면 반드시 밝힐 것.
블록2 — 어떨 때 쓰나: 손이 멈추는 상황을 장면으로.
블록3 — 뭐가 좋아지나: 쓰기 전과 후의 차이. 재료에 수치가 있으면 그대로 쓸 것.
...
- igCaption: 인스타 캡션 — **카드뉴스를 다 넘겨본 사람에게 이어서 말을 거는 글**. 요약 리포트가 아니다.
  ...
  ⚠️ **0단계에서 정의한 edge가 캡션에서 반드시 드러날 것.** 블록 3개 중 최소 하나는 edge를 말하는 자리다.
  ...
  ② 본문 — "▪ + 소제목 한 줄" 블록을 **정확히 3개**(가이드 소재만 2개). 블록당 **1문장**(길어도 2문장).
     블록 사이는 빈 줄로 띄운다. 각 블록이 무엇을 맡는지는 아래 [캡션 블록 역할]을 그대로 따를 것.
     ⚠️ **이모지를 쓰지 말 것** — 블록 마커는 ▪ 하나로 통일한다(소제목 앞에만, 문장 속엔 넣지 않는다).
     매 블록에 다른 이모지를 다는 건 금지. 캡션 전체에 이모지는 0개다.
