...
/**
 * 마케팅 유입 링크(자동DM 숏링크) 생성 폼 + 행 액션.
 *
 * UtmBuilderForm과 같은 utm_links 테이블을 쓰되 kind='dm'으로 저장하고
 * 숏링크 code·게시물/키워드/자동화 대장 필드를 함께 기록한다.
 * 리틀리 자동DM 버튼에는 {SITE}/l/{code}만 넣는다 — 본가 redirect handler가
...
export function MarketingLinkForm({ siteBase }: { siteBase: string }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setF({
      ...
      custom_code: '', source: 'instagram', medium: 'dm', campaign: '', content: '',
    });
    router.refresh();
  }
    ...
    <form onSubmit={onSubmit} className="card p-5 mb-6 space-y-3">
      ...
      <details className="text-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-3">
          ...
          <Field label="자동화 이름"><Input value={f.flow_name} onChange={(e) => set('flow_name', e.target.value)} placeholder="자료 요청 DM" /></Field>
      ...
      <p className="text-xs text-ink/40">URL과 키워드만 넣으면 돼요 — 라벨·campaign은 키워드로, source·medium은 instagram/dm으로 자동 저장됩니다.</p>
      {error && <p className="text-sm text-red-600">{error}</p>}
      {built && (
        <div className="rounded-md bg-muted p-3 text-xs">
          <div className="text-ink/50 mb-1">생성됨 — 이 숏링크를 리틀리 DM 버튼에 넣으세요</div>
