...
# E. 폰으로 보내기 (모바일에서 직접 올릴 때)

발행 패널의 **[폰으로 보내기]** = 카드를 렌더·업로드한 뒤 **서명된 링크**를 준다.
그 링크를 폰으로 옮기면(카톡 '나에게 보내기' 등) 카드 전 장 + 캡션 복사 버튼이 한 화면에 뜬다.
zip은 맥에 떨어지고 폰으로 옮기는 단계가 남는데, 그 구멍을 메우는 용도다.

- 링크: `/m/{카드id}?t={서명}` — 서명은 `DRAFT_PREVIEW_SECRET` 기반 HMAC(`lib/cardpress/handoff.ts`).
  틀리거나 없으면 **404**(권한 오류가 아니라 없는 페이지처럼 군다).
- 로컬 dev면 `localhost` 대신 **맥의 LAN 주소**를 준다 — localhost 링크는 폰에서 폰 자신을 가리켜 안 열린다.
  즉 **같은 와이파이**에 있어야 한다. prod 배포본은 아무 데서나 열린다.
- 발행·zip과 **같은** 렌더 파이프라인이라 폰에서 보는 그림과 실제 발행물이 어긋나지 않는다.
- 누를 때마다 그 카드 폴더에서 **이전 렌더의 잔재를 지운다.** 파일명에 템플릿이 들어가서
  (`03_B2.png` → `03_P2.png`) 슬라이드를 바꾸면 옛 파일이 같은 자리에 남고, 폰 화면이 폴더를
  그대로 읽으므로 옛 카드가 섞여 보이기 때문. 발행된 게시물은 Meta가 자체 복사본을 갖고 있어 영향 없다.

## E-부록. 인스타 DM으로 보내기 — **현재 막힘** (2026-08-18 확인)

원래는 카드+캡션을 **내 인스타 DM으로 직접** 보내려 했다. 코드는 다 있는데(아래) 동작하지 않는다.

**막힌 지점**: DM 전송에는 **IGSID(Instagram-scoped ID)** 가 필요한데,
Meta 문서상 이 값의 유일한 출처가 **echo 웹훅**이고, 그 웹훅이 **한 건도 배달되지 않는다.**

실측으로 확인한 것 (같은 실수를 반복하지 않기 위해 남긴다):

| 확인한 것 | 결과 |
|---|---|
| 로그인 id·비즈니스 id로 DM 전송 | `요청한 사용자를 찾을 수 없습니다` (code 100 / **2534014**) — 이 둘은 IGSID가 아니다 |
| Conversations API로 '나와의 대화' 조회 | **0건.** 폴더(inbox·requests)·필드 조합 전부. 인스타가 자기 자신과의 스레드를 API에서 제외한다 |
| 공개 URL(터널) + 웹훅 구독 | 구독 등록 시 Meta가 GET 확인을 통과 → **도달은 확실** |
| 실제 셀프 DM · Meta 자체 테스트 페이로드 | **POST 0건.** 우리 라우트로는 직접 쏘면 정상 수신됨(우리 쪽 문제 아님) |
| Instagram 테스터 역할 | 이미 추가·수락 완료 상태였음 — 그래도 결과 동일 |
| 앱 상태 | `dev_mode` · `instagram_business_manage_messages` = **access_level: none** |

**결론**: 발행 API는 dev 모드에서 내 계정에 한해 열려 있지만 **메시지 웹훅 배달은 그 예외에 없다.**
앱을 Live로 올리는 것(개인정보처리방침 URL + 검수) 외에 우회로가 없다 → 검수 금지 방침이라 여기서 멈춘다.

> Live로 올리게 되면 그대로 되살아나는 것들:
> `lib/cardpress/dm.ts`(Self Messaging 전송 + 캡션 1000바이트 분할),
> `app/api/instagram/webhook/route.ts`(IGSID 캡처), `scripts/instagram-self-igsid.mjs`.
> 순서는 ① 웹훅 콜백 등록 → ② 인스타 앱에서 나에게 DM → ③ 잡힌 IGSID를 `INSTAGRAM_SELF_IGSID` 에 기록.
> 웹훅 라우트는 값을 **모듈 전역이 아니라 임시 파일**에 남긴다 — Next dev가 요청마다 라우트 모듈을
> 새로 평가해서 POST가 담아둔 전역 변수가 GET에서 안 보인다(이걸로 한참 헤맸다).
...
| `'나와의 대화'를 찾지 못했습니다` | DM 경로는 현재 막혀 있다 | E-부록 참고 — 폰 전달은 [폰으로 보내기] 링크로 |
| `요청한 사용자를 찾을 수 없습니다` (code 100/2534014) | IGSID 자리에 로그인 id·비즈니스 id를 넣음 | 이 둘은 IGSID가 아니다 (E-부록) |
| 폰에서 링크가 안 열림 | 로컬 dev 주소(같은 와이파이 전용)를 다른 망에서 열었음 | 같은 와이파이로 붙거나 prod 배포본 사용 |
| 폰 화면에 옛 카드가 섞여 보임 | 이전 렌더 잔재 (수정 전 버그) | [폰으로 보내기]를 다시 누르면 폴더를 정리하고 다시 만든다 |
| `N번째 카드에서 중단 (M장 도착)` | DM 전송 중 실패 — 이미 간 M장은 그대로 남는다 | 원인 해결 후 다시 누르면 **처음부터** 다시 온다(중복분은 손으로 정리) |
